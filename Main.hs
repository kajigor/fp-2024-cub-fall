...
multByIndex :: [Int] -> [Int]
multByIndex list = zipWith (*) list [0..]

powerByIndex :: [Int] -> [Int]
powerByIndex list = zipWith (^) list [0..]

productOfDifference :: [Int] -> Int
productOfDifference [] = error "list should be at least size 2 got 0"
productOfDifference [_] = error "list should be at least size 2 got 1"
productOfDifference [a, b] = a - b
productOfDifference (x:y:xs) = (x - y) * productOfDifference (y:xs)

isSorted :: [Int] -> Bool
isSorted [] = True
isSorted [_] = True
isSorted (a:b:xs) = (b >= a) && isSorted (b:xs)

countElement :: Int -> [Int] -> Int
countElement x xs = length (filter (== x) xs)

dotProduct :: [Int] -> [Int] -> Int
dotProduct ls1 ls2 = sum (zipWith (*) ls1 ls2)

applyAll :: [a -> b] -> a -> [b]
applyAll [] _ = []
applyAll (func : functionList) x = func x : applyAll functionList x
